--Name : Survie dans un champ de mine

function init()
    player = PlayerSpaceship():setFaction("Vindh"):setTemplate("AD-3"):setCallSign("Jc4A"):setPosition(11900, 7600);
    player:setHull(250)
    ...
    station:sendCommsMessage(player, "Vous etes scrap, votre ingenieur est mort. Venez me voir au waypoint 2 que je vous repare. Il y a des supply au waypoint 1");
...
function generateMinefield(x, y, width, height)
    for i=0, width - 1,1 do
        for j=0, height - 1,1 do
            local offset = j % 2 * 800
            Mine():setPosition(x + i * 1800 + offset,y + j * 1800);
        end
    end
end
